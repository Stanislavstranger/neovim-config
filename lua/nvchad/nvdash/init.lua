-- Local override for nvchad.nvdash.
-- The upstream module can call nvim_win_set_cursor() without a position when
-- the cursor is not currently on a dashboard button line.
local M = {}
local api = vim.api
local fn = vim.fn
...
local function nearest_key_line(key_lines, curline)
  local nearest_idx = 1
  local nearest_distance = math.huge

  for i, v in ipairs(key_lines) do
    local distance = math.abs(v.i - curline)
...
M.open = function(buf, win, action)
  ...
  local key_movements = function(n, cmd)
    ...
    current_idx = current_idx or nearest_key_line(key_lines, curline)

    local target_idx = current_idx + n
    if target_idx < 1 then
      target_idx = #key_lines
    elseif target_idx > #key_lines then
      target_idx = 1
    end

    local x = key_lines[target_idx]
